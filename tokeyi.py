...
if df_res is not None and not df_res.empty:
    ...
    with tab4:
        st.subheader("全ソースコード一括コピー & 表示 (Python / Streamlit)")
        st.caption("右上にある『📋 コピー』アイコンをクリックするか、以下のテキストエリアで全ソースコードを一括コピーできます。")
        ...
        full_code_text = ""
        for label, filepath in files_to_export:
            if os.path.exists(filepath):
                with open(filepath, "r", encoding="utf-8") as f:
                    content = f.read()
                    full_code_text += f"# ==========================================\n# FILE: {filepath}\n# ({label})\n# ==========================================\n\n{content}\n\n"

        # Streamlit標準のst.code (右上ホバーでコピーボタン自動表示)
        st.markdown("##### 📋 全コード一括コピー用コードブロック")
        st.code(full_code_text, language="python")

        st.markdown("---")
        st.markdown("##### 📁 個別ファイル別ソースコード")
        for label, filepath in files_to_export:
            if os.path.exists(filepath):
