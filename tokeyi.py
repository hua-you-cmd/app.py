...
if df_res is not None and not df_res.empty:
    ...
    # TAB 4: Python Full Code Exporter (Pythonコード専用コピー)
    with tab4:
        st.subheader("🐍 Python専用 ソースコード一括表示 & コピー")
        st.caption("コードブロックの右上にある『📋 コピー』アイコンをクリックすると、Pythonコードを一括コピーできます。")
        ...
        if os.path.exists("tokeyi.py"):
            ...
            st.success("💡 1つのPythonファイルのみで動作させたい場合は、こちらの `tokeyi.py` コード枠右上にある『📋 コピー』ボタンをご使用ください。")
        ...
        st.info("💡 以下のコード枠の右上ホバーで出現する『📋 コピー』ボタンを押すと、Python関連全コードを一括でコピーできます。")
        st.code(python_code_text, language="python")

        st.markdown("---")
        st.markdown("### 📄 3. Python個別ファイル別ソースコード")
        for label, filepath in python_files:
            if os.path.exists(filepath):
