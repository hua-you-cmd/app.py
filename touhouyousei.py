...
import { X, Copy, Check, Code, FileCode, Search, Download, Layers } from 'lucide-react';
import { playClickSound } from '../utils/sound';
...
export const CodeCopyModal: React.FC<CodeCopyModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleDownload = () => {
    ...
    element.download = selectedFile === 'ALL' ? 'oriental_yojo_all_code.txt' : selectedFile.replace('/', '_');
      ...
      <div className="bg-stone-900 border border-amber-500/40 rounded-3xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl text-stone-100 overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-stone-800 flex flex-wrap items-center justify-between bg-stone-900/90 shrink-0 gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-stone-400">
                アプリを構成する全コード（{fileList.length} ファイル / 計 {allCombinedCode.split('\n').length} 行）を直接コピー・ダウンロードできます
              </p>
            </div>
          </div>
            ...
            >
              {copied ? (
                ...
                <>
                  ...
                  <span>
                    {selectedFile === 'ALL' ? '全コードを一括コピー' : 'このファイルをコピー'}
                  </span>
                </>
              )}
            </button>
          ...
          <div className="md:col-span-4 lg:col-span-3 p-4 bg-stone-950/60 flex flex-col gap-3 min-h-0 overflow-y-auto">
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="text-xs font-bold truncate">全ファイル統合 (ALL)</div>
                <div className="text-[10px] text-amber-300/80">全 {fileList.length} ファイル一括テキスト</div>
              </div>
            </button>

            {/* File list */}
            <div className="text-[11px] font-bold text-stone-400 px-1 pt-2 flex justify-between items-center">
              <span>個別ファイル選択 ({filteredFiles.length})</span>
            </div>
            ...
            <div className="space-y-1.5 overflow-y-auto pr-1">
              {filteredFiles.map((f) => (
                <button
                  ...
                  className={`w-full px-3 py-2 rounded-xl text-left flex items-center gap-2.5 transition cursor-pointer text-xs ${
                    selectedFile === f.path
                ...
                >
                  <FileCode className="w-4 h-4 shrink-0 opacity-70 text-amber-400" />
          ...
          <div className="md:col-span-8 lg:col-span-9 flex flex-col bg-stone-950 min-h-0 overflow-hidden">
            <div className="px-4 py-2.5 bg-stone-900/80 border-b border-stone-800 flex items-center justify-between text-xs text-stone-400 font-mono shrink-0">
              <span className="text-amber-300 font-bold">
                {selectedFile === 'ALL' ? '全ソースコード（すべてのファイルを結合したコード）' : selectedFile}
              </span>
        ...
        <div className="px-6 py-3 border-t border-stone-800 bg-stone-900/95 flex items-center justify-between text-xs text-stone-400 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span>全コンポーネント・データ定義・ユーティリティ・CSSが含まれています</span>
          </div>
