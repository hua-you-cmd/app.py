...
def analyze_all_gmo_pairs(period: str = "1y", target_pips: float = 250.0, history_df: pd.DataFrame = None) -> list:
    ...
    for pair in SBI_PAIRS:
        ...
        if not df.empty:
            ...
            results.append({
                ...
                "price": pred.get("latest_close", 0.0),
                "long_prob": pred.get("long_prob", 50.0),
                "short_prob": pred.get("short_prob", 50.0),
    ...
    # 未確定ログの評価更新
    evaluate_prediction_outcomes(pairs_data_map)
